// batchArbWithMultiplierFix.js  
// Full drop-in script with multiplier fix for TRADE_MULTIPLIER  
// Requires Node.js and ethers v5+  
// Ensure you have dotenv, ethers installed: npm i dotenv ethers  
...
async function findProfitableTrade(buyRouter, sellRouter, tokenAddr) {  
  ...
  // Multiply amountIn by MULTIPLIER using fixed-point arithmetic to avoid FP issues.  
  // We'll scale multiplier by 1_000_000 and perform integer math.  
  const multiplierScaled = Math.floor(MULTIPLIER * 1_000_000); // e.g., 1.25 -> 1250000  
  const amountInAdjusted = amountIn.mul(multiplierScaled).div(1_000_000);  

  // Final amount to use  
  const amountInFinal = amountInAdjusted;  
