...
// 🟢 Vault-only arbitrage bot with full logging
...
const arbContract = new ethers.Contract(CONTRACT_ADDRESS, arbAbi, wallet || provider);
let usdcContract;
...
async function priceSanityCheck(routerAddr, token, amountUSDC) {
  try { return (await getAmountOut(routerAddr, token, amountUSDC)) > 0; }
  catch { return false; }
}
...
async function executeTradeLive(buyRouter, sellRouter, tokenAddr, amountUSDC) {
  ...
  try {
    ...
    if (expectedProfitUSDC <= MIN_EXPECTED_PROFIT || expectedProfitPct > MAX_PROFIT_PCT) return;

    if (!await priceSanityCheck(buyRouter, tokenObj, amountUSDC) ||
        !await priceSanityCheck(sellRouter, tokenObj, amountUSDC)) return;

    const minReturnUSDC = ethers.parseUnits((amountUSDC + expectedProfitUSDC * (1 - SLIPPAGE_PCT/100)).toString(), 6);

    console.log("\n🔍 ---------- New Trade Attempt ----------");
    console.log(`🔹 ${timestamp} • Token: ${tokenAddr} • AmountIn: ${amountUSDC} USDC`);
    console.log(`🏦 Vault Balance Before: ${fmt(before)} USDC`);
    console.log(`💹 DEX: ${Object.keys(routers).find(k=>routers[k]===buyRouter)} → ${Object.keys(routers).find(k=>routers[k]===sellRouter)}`);
    ...
    try {
      await provider.call({
        to: CONTRACT_ADDRESS,
        data: arbContract.interface.encodeFunctionData("executeArbitrage", [buyRouter, sellRouter, tokenAddr, ethers.parseUnits(amountUSDC.toString(), 6), minReturnUSDC]),
        from: wallet ? wallet.address : undefined
      });
      console.log("🔬 Simulation PASS ✅");
    } catch {
      console.log("❌ Simulation FAIL");
      return;
    }

    if (DRY_RUN) return;

    const gasEstimate = await arbContract.estimateGas.executeArbitrage(buyRouter, sellRouter, tokenAddr, ethers.parseUnits(amountUSDC.toString(), 6), minReturnUSDC);
    const tx = await arbContract.executeArbitrage(buyRouter, sellRouter, tokenAddr, ethers.parseUnits(amountUSDC.toString(), 6), minReturnUSDC, { gasLimit: gasEstimate.mul(120).div(100) });
    ...
    logTradeCSV({ timestamp, symbol, buyRouter: Object.keys(routers).find(k=>routers[k]===buyRouter), sellRouter: Object.keys(routers).find(k=>routers[k]===sellRouter), amount: amountUSDC, profitUSDC: netProfit });
    console.log(`✅ Trade SUCCESS — Real NET Profit: ${fmt(netProfit)} USDC`);
  } catch (err) {
  ...
  for (const [symbol, token] of Object.entries(tokens)) {
    for (const [buyName, buyRouter] of Object.entries(routers)) {
      for (const [sellName, sellRouter] of Object.entries(routers)) {
        ...
        try { await executeTradeLive(buyRouter, sellRouter, token.address, TRADE_AMOUNT_USDC); } catch (err) {
          console.log(`${symbol} | ${buyName}→${sellName} | scan error: ${err.message}`);
        }
      }
    }
  }
...
// ---------- MAIN ----------
console.log("🚀 Vault-only arbitrage runner started");
setInterval(async () => {
