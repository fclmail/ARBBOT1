// arbitrage-hardcoded-vault-live-protected.js
...
// Safety parameters (tuned for a tiny live test)
const MIN_PROFIT_PCT = Number(process.env.MIN_PROFIT_PCT || 1.5); // percent
const MIN_TRADE_USDC = Number(process.env.MIN_TRADE_USDC || 0.005); // allow very small test trades
const SLIPPAGE_PCT = Number(process.env.SLIPPAGE_PCT || 0.5); // 0.5% slippage tolerance
const MIN_EXPECTED_PROFIT = Number(process.env.MIN_EXPECTED_PROFIT || 0.01); // at least this many USDC expected (including gas)
...
async function getAmountOut(routerAddr, token, amountUSDC) {
  ...
  try {
    ...
    // Sanity: non-zero, not extremely large/small
    if (!out || !isFinite(out) || out <= Number.EPSILON) return 0;
    const ratio = amountUSDC / out; // price
    if (!isFinite(ratio) || ratio <= 0 || ratio > 1e6) return 0; // garbage filter
    return out;
  } catch (err) {
    return 0;
  }
...
// estimate gas cost in USDC by getting gasUsed * gasPrice (native) and converting to USDC via WMATIC->USDC quote
async function estimateGasCostUSDC(buyRouter, sellRouter, tokenAddr, amountUSDC) {
  try {
    ...
    const gasUsed = Number(gasEstimate.toString());
    const gasPriceBN = BigInt(gasPrice.toString());
    const gasCostNative = (BigInt(gasUsed) * gasPriceBN); // in wei of native token (MATIC)
    ...
    return Infinity; // can't estimate -> treat as too expensive
  }
}
...
async function executeTradeLive(buyRouter, sellRouter, tokenAddr, amountUSDC) {
  ...
  // get soft quotes for sanity checks
  ...
  if (!buyOut || !sellOut) { console.log(`'❌ Skipping — failed to get price'); return; }

  const buyPrice = amountUSDC / buyOut; // USDC per token
  const sellPrice = amountUSDC / sellOut; // USDC per token
  ...
  // estimate gas cost in USDC (on-chain reflective)
  const estGasUSDC = await estimateGasCostUSDC(buyRouter, sellRouter, tokenAddr, amountUSDC);
  if (!isFinite(estGasUSDC)) { console.log(`'❌ PREVENTED — cannot estimate gas cost (safety)'); return; }
  console.log(`⛽ Estimated gas: ${fmt(estGasUSDC,6)} USDC);

  // require expectedProfit > gas + min expected
  ...
  // require percent profit threshold
  const profitPct = (expectedProfitUSDC / amountUSDC) * 100;
  if (profitPct < MIN_PROFIT_PCT) { console.log(`❌ PREVENTED — profit pct ${fmt(profitPct,4)}% < MIN_PROFIT_PCT ${MIN_PROFIT_PCT}%); return; }

  // On-chain simulation using callStatic to ensure the vault will accept and profit
  try {
  ...
  for (const token of Object.values(tokens)) {
    for (const buyRouter of Object.values(routers)) {
      for (const sellRouter of Object.values(routers)) {
        ...
        try { await executeTradeLive(buyRouter, sellRouter, token.address, tradeAmountUSDC); }
