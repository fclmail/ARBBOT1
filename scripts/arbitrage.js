...
const MIN_TRADE_USDC = Number(process.env.MIN_TRADE_USDC || 1000.0);    
...
async function executeTradeLive(buyRouter, sellRouter, tokenAddr, amountUSDC) {
  ...
  try {
    ...
    const minReturnUSDC = ethers.parseUnits(
      (amountUSDC * (1 - SLIPPAGE_PCT / 100)).toFixed(6),
      6
    );
...
// ---------- SCAN LOOP ----------
const TRADE_AMOUNT_USDC = Number(process.env.TRADE_AMOUNT_USDC || 1000.0);
