...
const MIN_PROFIT_PCT = Number(process.env.MIN_PROFIT_PCT || 1);     // min % (script-level)
const MIN_TRADE_USDC = Number(process.env.MIN_TRADE_USDC || 0.001);    // Don't trade < this USDC
const GAS_EST_USDC = Number(process.env.GAS_EST_USDC || 0.002);       // conservative gas estimate in USDC
const MIN_EXPECTED_PROFIT = Number(process.env.MIN_EXPECTED_PROFIT || 0.001); // very small floor
const SLIPPAGE_PCT = Number(process.env.SLIPPAGE_PCT || 0.05);
