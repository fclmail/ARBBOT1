...
function getRouter(r){
if(!routerCache.has(r)){
routerCache.set(r,new ethers.Contract(r,routerAbi,provider));
}
return routerCache.get(r);
}
...
const MIN_PROFIT =
ethers.parseUnits(MIN_PROFIT_THRESHOLD.toString(),6);

const HOPS = [[],[WETH],[WMATIC],[USDT],[DAI]];
...
const buyPath = [USDC, ...hop, token];
const sellPath = [token, ...hop.slice().reverse(), USDC];

/* ===== PROMISE.ALL QUOTES ===== */

const [buyOut] = await Promise.all([
quote(buy, amountIn, buyPath)
]);

if (!buyOut) return;

const [sellOut] = await Promise.all([
quote(sell, buyOut, sellPath)
]);

if (!sellOut) return;

const profitBig = sellOut - amountIn;

if (profitBig <= MIN_PROFIT) return;

const profit = Number(
ethers.formatUnits(profitBig,6)
);
