...
/* ================= QUOTE WITH CACHING & DEDUPLICATION ================= */
async function quote(router, amount, path) {
  const cacheKey = `${router}:${amount.toString()}:${path.join("-")}`;
  const now = Date.now();

  // Return cached result or await existing promise
  ...
  const fetchPromise = (async () => {
    try {
      ...
      const val = out.at(-1);
      quoteCache.set(cacheKey, { value: val, timestamp: Date.now() });
      return val;
    } catch {
  ...
  quoteCache.set(cacheKey, { promise: fetchPromise, timestamp: now });
...
/* ================= MAIN ================= */
(async function main() {
  console.log("🚀 BOT STARTED WITH CACHED & CHUNKED PROMISE.ALL\n");
  provider = newProvider();
  rebuildContracts();

  const triangularPaths = buildTriangularPaths();
  const pathChunks = chunkArray(triangularPaths, CHUNK_SIZE);
  let batch = [];
  ...
  while (true) {
    for (const r of Object.values(routers)) {
      for (const chunk of pathChunks) {
        ...
        // Process a chunk of paths concurrently using Promise.all
        const results = await Promise.all(
          chunk.map((path) => findTriangular(r, path))
        );

        // Filter out nulls and append valid trades
        ...
        // Throttle control to respect RPC rate limits
        if (CHUNK_DELAY_MS > 0) {
