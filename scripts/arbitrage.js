...
const MIN_TRADE_USDC = Number(process.env.MIN_TRADE_USDC || 105); // input USDC
const MIN_EXPECTED_PROFIT = Number(process.env.MIN_EXPECTED_PROFIT || 0); // USDC
...
/* ===== HELPER: read vault USDC balance ===== */
async function vaultUSDCBalance() {
  ...
  return await usdcContract.balanceOf(VAULT_ADDRESS);
}
...
async function tryArb(buyRouter, sellRouter, tokenAddr, buyPath = null, sellPath = null) {
  ...
  // Log updated vault USDC balance
  const vaultBalance = await vaultUSDCBalance();
  console.log(`💰 Vault USDC balance: ${formatUSDC(vaultBalance)}`);
...
async function scan() {
  ...
  const vaultUSDC = Number(ethers.formatUnits(await vaultUSDCBalance(), 6));
