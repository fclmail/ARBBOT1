...
async function simulate(pathSymbols, routerA, routerB){
  ...
  try{
    ...
    const out1 = await router1.getAmountsOut(
      amountIn,
      pathAddresses.slice(0,2)
    )
    ...
    const out2 = await router2.getAmountsOut(
      amountMid,
      pathAddresses.slice(1)
    )

    const returned = out2[out2.length - 1]

    const returnedUSDC = Number(
      ethers.formatUnits(returned, 6)
    )
    ...
    if(net > 0){
      ...
      console.log(chalk.green("PROFITABLE TRADE FOUND"))
      ...
      console.log("Net profit:", format(net))
