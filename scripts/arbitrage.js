// ─────────────────────────────────────────────
// 🔹 AAVE FLASH ARB BOT — Polygon (Fixed)
// ─────────────────────────────────────────────
...
const MIN_NET_PROFIT_USDC = 1; // minimum net profit after gas
...
// ─────────────── SETTINGS ───────────────
const TRADE_AMOUNT_USDC = 1; // normalize 1 USDC
...
async function getAmountOut(routerAddr, token, amountIn) {
  ...
  try {
    ...
    try {
      ...
      console.warn(`⚠️ getAmountsOut failed for ${token.address}: ${err2.message}`);
      return null;
    }
  }
}
...
async function executeTrade(buyRouter, sellRouter, tokenAddr, amount) {
  try {
    ...
    const netProfit = Number(ethers.formatUnits(balanceAfter, 6)) - amount;
    cumulativeProfit += netProfit;

    console.log(`💹 Net USDC this tx: ${fmt(netProfit)} USDC`);
