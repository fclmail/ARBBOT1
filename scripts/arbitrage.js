// ─────────────────────────────────────────────
// 🟢 AAVE FLASH ARB BOT — Polygon (Safe Dry-Run)
// ─────────────────────────────────────────────
...
// ─────────────── EXECUTE TRADE WITH SAFETY 🟢9 ───────────────
async function executeTrade(buyRouter, sellRouter, tokenAddr, amount) {
  try {
    // ⚠️ Skip if executeArbitrage undefined
    if (!arbContract.executeArbitrage) {
      console.warn("⚠️ executeArbitrage not defined — skipping trade");
      return;
    }
    ...
    console.log(`💹 Net USDC this tx: ${netProfit.toFixed(6)} USDC`);
      ...
      for (const [sellName, sellRouter] of Object.entries(routers)) {
        ...
        try {
          ...
          if (profitPct >= MIN_PROFIT_PCT) {
            ...
            // Execute only if contract supports it
