...
//    (NO REAL TRANSACTIONS, SAFE MODE)
...
const CONTRACT_ADDRESS = "0x19B64f74553eE0ee26BA01BF34321735E4701C43"; // Existing vault contract
const MIN_NET_PROFIT_USDC = 2;

// Provider only (no wallet)
...
async function getAmountOut(routerAddr, token, amountIn) {
  // Simulate amounts based on random fluctuation
  const priceFluctuation = 0.98 + Math.random() * 0.04; // ±2%
  return amountIn * priceFluctuation;
}
...
async function executeTradeSimulated(buyRouter, sellRouter, tokenAddr, amount) {
  ...
  const symbol = Object.entries(tokens).find(([k,t])=>t.address===tokenAddr)[0];
  ...
  console.log(`🧪 ${symbol} | Buy: ${buyRouter} → Sell: ${sellRouter} | Amount: ${amount} USDC`);
...
async function scan() {
  ...
  console.log(`🔍 Scan complete. Found ${opportunities.length} simulated profitable opportunities.\n`);
  saveCSV();
