...
const ESTIMATED_GAS_COST_USDC = 0.001; // Estimated gas cost per trade in USDC
...
async function getExpectedProfit(buyRouterAddr, sellRouterAddr, tokenObj, tradeAmountUSDC) {
  ...
  try {
    ...
    const profitBN = usdcOutBN - amountInBN; // ethers v6 returns BigInt
    const profitUSDC = Number(profitBN) / 10 ** USDC_DECIMALS;
    ...
    return { profitUSDC: -9999, profitPct: -9999, amountInBN, usdcOutBN: 0n };
  }
}
...
async function simulateAndExecute(buyRouterAddr, sellRouterAddr, tokenObj, amountInBN, minReturnUSDCBN) {
  ...
  const profitRealUSDC = Number(afterBN - beforeBN) / 10 ** USDC_DECIMALS;
