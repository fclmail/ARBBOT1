/* ============================================================
   ArbJS - Full example, ES module, ethers v6 compatible
   - Polygon environment assumed
   - Logs balances, simulates, and executes profitable arbitrage
   - Vault, tokens, routers, and error handling included
   ============================================================ */

import { Wallet, JsonRpcProvider, Contract, utils } from "ethers";

// --------------------------- CONFIG ---------------------------
const INTERVAL = 15000;       // 15 seconds scan
const SLIPPAGE_BPS = 20;      // slippage basis points
const JS_MIN_PROFIT = 0.01;   // minimum profit in USDC
const TRADE_USDC = 0.5;       // USDC per trade

// Vault and USDC contract addresses
const USDC = "0x2791bca1f2de4661ed88a30c99a7a9449aa84174";
const VAULT = "0x2dD5820519aBbC74DB5658744e9EbAf9ED88320e";

// Wallet configuration: private key in secrets/environment
const wallet = {
...
function usdcAmount(amount) {
  return utils.parseUnits(Number(amount).toFixed(6), 6);
}
...
async function displayBalances() {
  try {
    ...
    const walletUSDCRaw = await USDC_CONTRACT.balanceOf(wallet.address);
    const walletUSDC = toUSDC(walletUSDCRaw);

    const vaultUSDCRaw = await USDC_CONTRACT.balanceOf(VAULT);
    const vaultUSDC = toUSDC(vaultUSDCRaw);

    console.log(`💠 Wallet MATIC balance: ${Number(nativeEth).toFixed(6)} MATIC`);
    console.log(`💠 Wallet USDC balance: ${walletUSDC.toFixed(6)} USDC`);
    console.log(`💠 Vault USDC balance: ${vaultUSDC.toFixed(6)} USDC`);
  } catch (e) {
      ...
      for (const buy of DEXES) {
        for (const sell of DEXES) {
          ...
          try {
            ...
            console.log(`💠 Wallet USDC balance (post-arb): ${walletUSDCPost.toFixed(6)} USDC`);
            console.log(`💠 Vault USDC balance (post-arb): ${vaultUSDCPost.toFixed(6)} USDC`);
