...
async function quote(routerAddr, amountIn, path) {
  try {
    ...
    // Basic sanity: amountOut must be > 0
    if (!amountOut || amountOut.lte(0)) {
      console.log(
        `${RED}⚠️ Quote returned non-positive amount| Router: ${routerAddr} | Path: ${path?.map((p) => p).join('->')}${RESET}`
      );
    ...
    console.log(
      `${RED}⚠️ Quote failed| Router: ${routerAddr} | Path: ${path?.map((p) => p).join('->') || 'unknown'} | Error: ${e?.message || e}${RESET}`
    );
    return { amountOut: null, ok: false, path, router: routerAddr };
  }
}
...
async function findOptimalTradeSize(buyRouter, sellRouter, tokenAddr, buyPath, sellPath) {
  ...
  while (high - low > 0.001 && it < maxIterations) {
    ...
    if (!buyRes.ok || buyRes.amountOut == null) {
      console.log(
        `${RED}⚠️ Buy quote failed for path ${buyPath} on router ${buyRouter}${RESET}`
      );
      // If buy quote fails, skip further evaluation for this iteration
      continue;
    }
    ...
    if (!sellRes.ok || sellRes.amountOut == null) {
      console.log(
        `${RED}⚠️ Sell quote failed for path ${sellPath} on router ${sellRouter}${RESET}`
      );
      continue;
    }

    // All amounts are in base units; assume USDC with 6 decimals
  ...
  if (!Number.isFinite(bestSize) || Number.isNaN(bestSize)) {
    bestSize = MIN_TRADE_USDC;
  }
...
async function tryArb(buyRouter, sellRouter, tokenAddr, buyPath, sellPath) {
  ...
  // Compute best size and profit for this path pair
  ...
  for (let attempt = 0; attempt < TX_RETRY_ATTEMPTS; attempt++) {
    try {
      ...
      // Optional: deposit profits back to vault if you have a deposit function
      // await depositProfitsBackToVaultIfNeeded(profitAmountWei);
      break;
    } catch (err) {
...
/**
 * If your vault contract exposes a deposit function to pool profits back, enable this.
 * Example signature: function deposit(uint256 amount) -> deposits into vault.
 * Uncomment and implement according to your vault's actual API.
 */

// async function depositProfitsBackToVaultIfNeeded() {
...
/* Ensure we only consider valid paths (sanity-check) */
function isPathValid(path) {
  if (!path || !Array.isArray(path) || path.length < 2) return false;
  // Basic check: ensure each entry is an address-like string
...
/* ================= TOKENS ENDPOINT UTILITIES (for completeness) ================= */
// (Any additional helpers can be added here)

/* ================= ROUTER PATH DIAGNOSTICS (optional) ================= */
// You can log path validity per combination if you want deeper debugging
