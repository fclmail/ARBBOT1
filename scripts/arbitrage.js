// improved-arbitrage.js (DRY RUN ENABLED VERSION)
// Only change: DRY_RUN is forced to true. Everything else unchanged.
...
// ---------- CONFIG ----------
// Force dry run always ON
const DRY_RUN = true;
console.log("🔬 DRY RUN ENABLED — NO TRANSACTIONS WILL EXECUTE");
...
const ROUTERS = {
  ...
  quick: "0xa5e0829caecd0c0aa76d551d8e5f7f3d77a91fd3",
  firebird: "0xf3a9f0b736b3e7f5ce437d7b8da89f219f0adc55"
};
...
async function getPrice(routerAddr, tokenIn, tokenOut, amountIn) {
  try {
    const router = routerContract(routerAddr);
    const path = [tokenIn.address, tokenOut.address];
    const amt = ethers.parseUnits(amountIn.toString(), tokenIn.decimals);
    const out = await router.getAmountsOut(amt, path);
    return Number(ethers.formatUnits(out[1], tokenOut.decimals));
  } catch (e) {
    return null;
  }
}

async function estimateGasUSD() {
  try {
    const gas = await provider.estimateGas({ from: wallet.address, to: wallet.address, value: 0 });
    const gasPrice = await provider.getGasPrice();
    const maticPrice = 0.75;
    return Number(ethers.formatUnits(gas * gasPrice, 18)) * maticPrice;
  } catch (e) {
    return 0.10;
  }
}
...
async function checkArb() {
  ...
  const amount = TRADE_AMOUNT_USDC;

  for (const r1Name in ROUTERS) {
    for (const r2Name in ROUTERS) {
      ...
      const out2 = out1
        ? await getPrice(r2, tokenB, tokenA, out1)
        : null;
      ...
      // DRY RUN — No illegal return, no TX sent
      log("🚫 DRY RUN ENABLED — No transaction executed.");
      continue;
    }
  }
}
...
async function main() {
  while (true) {
    await checkArb();
    log(`Waiting ${SCAN_INTERVAL_MS / 1000}s...`);
