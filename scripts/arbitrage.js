...
const MIN_TRADE_USDC = Number(process.env.MIN_TRADE_USDC || 0.0020);
...
async function tryArb(buyRouter,sellRouter,tokenAddr,buyPath=null,sellPath=null){
  ...
  for(let attempt=0; attempt<TX_RETRY_ATTEMPTS; attempt++){
    try{
      ...
      tx = await vault.executeArbitrage(buyRouter,sellRouter,amountIn,directPathBuy,directPathSell,deadline,txOpts);
...
/* ================= AAVE LIQUIDITY FIXED ================= */
async function getAaveLiquidity(){
  const pool = new ethers.Contract(process.env.AAVE_POOL, [
    "function getReserveData(address asset) view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint40, uint16)"
  ], provider);

  const reserve = await pool.getReserveData(TOKENS.USDC);
  const availableLiquidity = reserve[1]; // BigInt

  console.log(`🏦 Aave Available USDC Liquidity: ${ethers.formatUnits(availableLiquidity,6)}`);
  return availableLiquidity;
}

function optimizeFlashAmount(liquidityBigInt){
  const optimal = liquidityBigInt * 70n / 100n;
  console.log(`⚙️ Optimal Flash Amount: ${ethers.formatUnits(optimal,6)} USDC`);
  return optimal;
}
...
async function scan(){
  ...
  const aaveLiquidity = await getAaveLiquidity();
  const optimalFlash = optimizeFlashAmount(aaveLiquidity);
