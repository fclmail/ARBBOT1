...
async function executeArb(buyRouter, sellRouter, token, profit) {
  const vaultBal = await getVaultBalance();
  if (vaultBal < TRADE_AMOUNT_USDC) {
    log(`❌ SKIP: Vault has insufficient USDC ${vaultBal.toFixed(6)}`);
    return;
  }

  log(`💰 EXECUTING ${profit.toFixed(6)} USDC`);
  ...
  const minReturn = amountIn.add(ethers.parseUnits(MIN_PROFIT_USDC.toString(), 6));
  ...
  try {
    ...
    log(`❌ EXECUTION FAILED: ${e.message}`, false);
  }
}
      ...
      for (const buyPath of buyPaths) {
        tasks.push((async () => {
          ...
          for (const sellPath of sellPaths) {
            ...
            const line =
              `[${new Date().toISOString()}] [${token.symbol}] BUY ${buyDex.name} → SELL ${sellDex.name} | P/L ${profit.toFixed(6)}`;

            log(line, profit > 0);

            if (profit >= MIN_PROFIT_USDC) {
              await executeArb(buyDex.contract.address, sellDex.contract.address, token.address, profit);
            }
          }
        })());
      }
...
async function main() {
  log("⏱ Polygon Arb Bot Started", true);
