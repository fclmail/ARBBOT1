...
/* ================= CONFIG ================= */
// Control batch size limits safely within 3 to 100 per your requirements
const BATCH_SIZE = 12; 
...
FRXETHANONICAL:"0xee327f889d5947c1dc1934bb208a1e792f953e96",
...
const getRouterName = (addr) => Object.keys(routers).find(k => routers[k] === addr) || addr.slice(0, 6);
...
async function findTriangular(router, path) {
    ...
    // Output string matching the clean telemetry parameters requested
    const routeStr = `USDC->${getSymbol(path[1])}->${getSymbol(path[2])}->USDC`.padEnd(55);
    const sizeStr = fmt(BASE_TRADE).padStart(6);
    const profitStr = (profit > 0n ? parseFloat(fmt(profit)) : 0.0000).toFixed(4);
    
    console.log(`⚡ MICRO SCAN -> Route: ${routeStr} | Size: ${sizeStr} USDC | Raw Return: ${profitStr} USDC`);
...
async function topUpGas() {
    try {
        ...
        const wmatic = new ethers.Contract(TOKENS.WMATIC, [
            "function withdraw(uint256)",
            "function balanceOf(address) view returns(uint256)"
        ], wallet);
...
(async function main() {
    ...
    // Track blocks dynamically to structure log outputs cleanly per runtime mining interval
    let lastBlock = 0;
    ...
    while (true) {
        try {
            ...
            if (currentBlock > lastBlock) {
                ...
                console.log(`\n🔔 NEW BLOCK MINED: #${currentBlock} | Initiating Parallel State Simulation...`);
            }
